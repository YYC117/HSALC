...
def main():
    ...
    index = np.argsort(np_train)    #元素从小到大排列，提取其对应的index(索引)
    ...
    recorder4 = [[] for i in range(e_h_dataset.__len__())]
    ...
    unconfirm1 = []
    for i in range(dirty_dataset.__len__()):
        if pre[i] != dirty_dataset.test_labels[i] and predicted[i] == 0: 
            unconfirm.append(i)
            unconfirm0.append(i)
        elif pre[i] == dirty_dataset.test_labels[i] and predicted[i] == 1: 
            unconfirm.append(i)
            unconfirm1.append(i)
    # label correction 
    for i in range(dirty_dataset.__len__()):
